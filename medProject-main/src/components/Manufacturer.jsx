...
export default function Manufacturer() {
  const [ammValidationResult, setAmmValidationResult] = useState(null);
  ...
  const creerLotMedicament = async () => {
    ...
    if (ammValidationResult?.status === 'FRAUD') {
      setTransactionStatus('error: AMM validation failed - potential fraud detected');
      return;
    }
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      const conditionsData = {
        tempMax: 0,
        tempMin: 0,
        tempAvg: parseInt(boxConditions.temperature) || 0,
        humidMax: 0,
        humidMin: 0,
        humidAvg: parseInt(boxConditions.humidite) || 0,
  ...
  <div className="relative flex items-center">
    <input
      ...
      onChange={async (e) => {
        ...
        try {
          // Step 1: Show validation in progress
          setTransactionStatus('Validating AMM document...');
          
          // Step 2: Perform AMM verification first
          const validationResult = await checkAMMFraud(file);
          setAmmValidationResult(validationResult);
          
          // Step 3: Check validation result before proceeding
          if (validationResult.status === 'FRAUD') {
            setTransactionStatus('error: AMM validation failed - potential fraud detected');
            // Clear the file input if fraud detected
            e.target.value = '';
            return;
          }

          // Step 4: Only proceed to IPFS upload if validation passed
          setTransactionStatus('Uploading analysis certificate to IPFS...');
          const cid = await uploadPDFToIPFS(file);
          
          // Step 5: Update the material with the CID
          ...
          // Step 6: Clear status on success
  ...
  {ammValidationResult && (
    <p className={`mt-1 text-xs ${
      ammValidationResult.status === 'VALID' ? 'text-green-600' : 'text-red-600'
    }`}>
      AMM Status: {ammValidationResult.status} (Confidence: {(ammValidationResult.probability * 100).toFixed(1)}%)
    </p>
  )}
  {transactionStatus?.includes('Uploading') && (
    <p className="mt-1 text-xs text-blue-600">{transactionStatus}</p>
  )}
</div>
